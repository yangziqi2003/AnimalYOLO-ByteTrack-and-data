...
2. Proceed to retrieve all label files for your dataset.
    ...
    labels = sorted(dataset_path.rglob("*labels_xml/*.txt")) # all data in 'labels_xml'
    ```
...
4. Next, we create the directories and dataset YAML files for each split.
    ...
    for split in folds_df.columns:
        ...
        (split_dir / 'train' / 'labels_xml').mkdir(parents=True, exist_ok=True)
        (split_dir / 'val' / 'images').mkdir(parents=True, exist_ok=True)
        (split_dir / 'val' / 'labels_xml').mkdir(parents=True, exist_ok=True)
...
5. Lastly, copy images and labels into the respective directory ('train' or 'val') for each split.
    ...
    for image, label in zip(images, labels):
        for split, k_split in folds_df.loc[image.stem].items():
            ...
            lbl_to_path = save_path / split / k_split / 'labels_xml'
