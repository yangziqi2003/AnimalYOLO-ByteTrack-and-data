...
def img2label_paths(img_paths):
    ...
    sa, sb = f'{os.sep}images{os.sep}', f'{os.sep}labels_xml{os.sep}'  # /images/, /labels_xml/ substrings
...
def verify_image_label(args):
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            if nl:
                if keypoint:
                    assert lb.shape[1] == (5 + nkpt * ndim), f'labels_xml require {(5 + nkpt * ndim)} columns each'
                    points = lb[:, 5:].reshape(-1, ndim)[:, :2]
                else:
                    assert lb.shape[1] == 5, f'labels_xml require 5 columns, {lb.shape[1]} columns detected'
                    points = lb[:, 1:]
                ...
                assert max_cls <= num_cls, \
                    ...
                    f'Possible class labels_xml are 0-{num_cls - 1}'
                _, i = np.unique(lb, axis=0, return_index=True)
                if len(i) < nl:  # duplicate row check
                    lb = lb[i]  # remove duplicates
                    if segments:
                        segments = [segments[x] for x in i]
                    msg = f'{prefix}WARNING ⚠️ {im_file}: {nl - len(i)} duplicate labels_xml removed'
            else:
                ne = 1  # label empty
...
class HUBDatasetStats:
    ...
    def get_json(self, save=False, verbose=False):
        ...
        def _round(labels):
            """Update labels_xml to integer class and 4 decimal place floats."""
            if self.task == 'detect':
