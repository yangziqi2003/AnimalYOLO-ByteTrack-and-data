...
def non_max_suppression(
    ...
    """
    Perform non-maximum suppression (NMS) on a set of boxes, with support for masks and multiple labels_xml per box.
    ...
    Args:
        ...
        multi_label (bool): If True, each box may have multiple labels_xml.
        labels (List[List[Union[int, float, torch.Tensor]]]): A list of lists, where each inner
            list contains the apriori labels_xml for a given image. The list should be in the format
    ...
    multi_label &= nc > 1  # multiple labels_xml per box (adds 0.5ms/img)
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Cat apriori labels_xml if autolabelling
...
def segments2boxes(segments):
    """
    It converts segment labels_xml to box labels_xml, i.e. (cls, xy1, xy2, ...) to (cls, xywh)
